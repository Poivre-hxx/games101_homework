...
void rst::rasterizer::rasterize_triangle(const Triangle& t) {
    ...
    double min_x = std::min(v[0].x(), std::min(v[1].x(), v[2].x()));
    double max_x = std::max(v[0].x(), std::max(v[1].x(), v[2].x()));
    double min_y = std::min(v[0].y(), std::min(v[1].y(), v[2].y()));
    double max_y = std::max(v[0].y(), std::max(v[1].y(), v[2].y()));
    ...
    if (MSAA == false) {    
        for (int x = min_x; x <= max_x; x++) {
            for (int y = min_y; y <= max_y; y++) {
                ...
                {    
                    // If so, use the following code to get the interpolated z value.I
        ...
        for (int x = min_x; x <= max_x; x++) {
            for (int y = min_y; y <= max_y; y++) {
                int count = 0;
                if (insideTriangle(x + 0.25, y + 0.25, t.v)) count++;
                if (insideTriangle(x + 0.75, y + 0.75, t.v)) count++;
                if (insideTriangle(x + 0.25, y + 0.75, t.v)) count++;
                if (insideTriangle(x + 0.75, y + 0.25, t.v)) count++;    
                if (count > 0) {
                    // If so, use the following code to get the interpolated z value.I
                    auto[alpha, beta, gamma] = computeBarycentric2D((float)x + 0.5, (float)y + 0.5, t.v);
                    float w_reciprocal = 1.0/(alpha / v[0].w() + beta / v[1].w() + gamma / v[2].w());
